...
#include "TPaveStats.h" //trying this
...
void run(string file){//, string file2){
  ...
  TH1F *h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut = new TH1F("h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut", "h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut", 50, 0, 10); h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut->SetXTitle("DeltaR between dimuon1vtx & dimuon2vtx immediately after dZ Cut");
  h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut->Sumw2();
  
  TH1F *h_Z_prob = new TH1F ("h_Z_prob", "h_Z_prob", 200, 0, 1); h_Z_prob->SetXTitle("Vertex Prob for Vertex Assigned as Z Vertex");
  h_Z_prob->Sumw2();
  
  TH1F *h_upsi_prob = new TH1F("h_upsi_prob", "h_upsi_prob", 200, 0, 1); h_upsi_prob->SetXTitle("Vertex Prob for Vertex Assigned as Upsi Vertex");
  h_upsi_prob->Sumw2();
  
   TH2F *h_dimuonVtx_deltaR_vs_dZ = new TH2F ("h_dimuonVtx_deltaR_vs_dZ", "h_dimuonVtx_deltaR_vs_dZ; deltaR_dimuon1vtx_dimuon2vtx; dZ_dimuon1vtx_dimuon2vtx", 50, 0., 10., 50, 0., 10.);
   h_dimuonVtx_deltaR_vs_dZ->Sumw2();
...
//  std::cout << "myAdd(3,5) "<< myAdd(3,5) << std::endl;
 ...
 // double mu_mu_from_Z_Prob_Cut = 0.05; 
  ...
  TFile *ntuple = new TFile("ntuple_skimmed_big4muVtxCutRemoved_dZCutAdded_23Feb2022_inputFileIs_Runs2016-2017-2018.root", "RECREATE");
  TTree *aux;
...
///////////////////////////
  ...
  for(int iEntry=0; iEntry<entries; iEntry++) {
    ...
    if (eventCounter % 1000 == 0){
      cout << "\r" << eventCounter << " processed" << flush;
...
//          }  
    ...
    //  std::cout << "dimuon1vtx_xpos  " << TREE->dimuon1vtx_xpos->at(i) << std::endl; 
      ...
      h_dimuonVtx_deltaR_vs_dZ->Fill(deltaR_dimuon1vtx_dimuon2vtx, dZ_dimuon1vtx_dimuon2vtx);
      
      if (dZ_dimuon1vtx_dimuon2vtx > 1){
        continue;
      }
      h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut->Fill(deltaR_dimuon1vtx_dimuon2vtx);
...
//         // std::cout << (lepton1 + lepton2).M()<< std::endl; 
         ...
         if (Z_first_upsi_phase1_second_pair_12_34_56) {
             ...
             h_Z_prob->Fill(TREE->dimuon1vtx->at(i));
             h_upsi_prob->Fill(TREE->dimuon2vtx->at(i));
 ...
 //          // std::cout << "temp_Z_phi.at(0): " << temp_Z_phi.at(0) << std::endl;
      ...
      //      
         ...
         if  (upsi_phase1_first_Z_second_pair_12_34_56) { 
             h_Z_prob->Fill(TREE->dimuon2vtx->at(i));
             h_upsi_prob->Fill(TREE->dimuon1vtx->at(i));
...
std::cout << "numOfQuadsLookedAt:  " << numOfQuadsLookedAt << std::endl; 
...
/////////////////////////////////////////////////////////
  ...
  //c_big4MuVtxProb_before_big4MuVtx_Prob_Cut->SetLogy();
...
//  h_big4MuVtxProb_before_big4MuVtx_Prob_Cut->SaveAs("h_big4MuVtxProb_before_big4MuVtx_Prob_Cut.pdf"); //this file will be corrupted if you try to save the hist as a pdf!!
  ...
  c_dimuon_vtx->cd(1)->SetLogy(); 
  h_dimuon1VtxProb_before_Cut->SetMinimum(1);
  h_dimuon1VtxProb_before_Cut->Draw("e1");
  
  c_dimuon_vtx->cd(2)->SetLogy();
  h_dimuon2VtxProb_before_Cut->SetMinimum(1);
  h_dimuon2VtxProb_before_Cut->Draw("e1");
  ...
  TCanvas *c_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut = new TCanvas("c_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut", "c_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut");
  c_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut->cd();
  h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut->Draw();
  h_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut->Write();
  c_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut->SaveAs("c_deltaR_dimuon1vtx_dimuon2vtx_immediatelyAfter_dZCut.pdf");
  
  TCanvas *c_Z_prob = new TCanvas("c_Z_prob", "c_Z_prob");
  c_Z_prob->cd();
  h_Z_prob->SetMinimum(1);
  h_Z_prob->Draw();
  c_Z_prob->SetLogy(1);
  h_Z_prob->Write();
  c_Z_prob->SaveAs("c_Z_prob.pdf");  
  
  TCanvas *c_upsi_prob = new TCanvas("c_upsi_prob", "c_upsi_prob");
  c_upsi_prob->cd();
  h_upsi_prob->SetMinimum(1);
  h_upsi_prob->Draw();
  c_upsi_prob->SetLogy(1);
  h_upsi_prob->Write();
  c_upsi_prob->SaveAs("c_upsi_prob.pdf");
  
  TCanvas *c_dimuonVtx_deltaR_vs_dZ = new TCanvas("c_dimuonVtx_deltaR_vs_dZ", "c_dimuonVtx_deltaR_vs_dZ");
  c_dimuonVtx_deltaR_vs_dZ->cd();
  h_dimuonVtx_deltaR_vs_dZ->SetMinimum(1);
  h_dimuonVtx_deltaR_vs_dZ->Draw("COLZ");
  c_dimuonVtx_deltaR_vs_dZ->SetLogz(1);
  h_dimuonVtx_deltaR_vs_dZ->Write();
 //TPaveStats *st = (TPaveStats*)h_dimuonVtx_deltaR_vs_dZ->GetListOfFunctions()->FindObject("stats");
// st->SetX1NDC(0.25);
// st->SetX2NDC(0.5);
  c_dimuonVtx_deltaR_vs_dZ->SaveAs("c_dimuonVtx_deltaR_vs_dZ.pdf");
