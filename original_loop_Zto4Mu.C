...
//  TH1F *h_dimuon_from_upsi_before_Cut  = new TH1F("h_dimuon_from_upsi_before_Cut ",  "h_dimuon_from_upsi_before_Cut ", 200, 0, 1);  h_dimuon_from_upsi_before_Cut ->SetXTitle("h_dimuon_from_upsi_before_Cut ");
  ...
  TH1F *h_cutflow_allQuadCuts = new TH1F("h_cutflow_allQuadCuts", "h_cutflow_allQuadCuts", 7, -0.5, 6.5); h_cutflow_allQuadCuts->SetXTitle("Cuts involving overall quad");
  h_cutflow_allQuadCuts->Sumw2();
  
  TH1F *h_cutflow_Z_first_upsi_phase1_second_pair_12_34_56 = new TH1F("h_cutflow_Z_first_upsi_phase1_second_pair_12_34_56","h_cutflow_Z_first_upsi_phase1_second_pair_12_34_56", 16, -0.5, 15.5); h_cutflow_Z_first_upsi_phase1_second_pair_12_34_56->SetXTitle("Cutflow for the Z_first_upsi_phase1_second_pair_12_34_56 case");
...
//  std::cout << "myAdd(3,5) "<< myAdd(3,5) << std::endl;
  ...
  double Z_mass_high = 80.;
  
  double upsi_mass_low_phase1 = 4;
  
  double upsi_mass_high_phase1 = 80;
  
  double upsi_mass_low_phase2 = 4;
  
  double upsi_mass_high_phase2 = 80; 
  
  double lead_mu_from_Z_pT_Cut = 20;
  
  double sublead_mu_from_Z_pT_Cut = 10;
  
  double mu_from_upsi_pT_Cut = 5; //We could lower this and gain back a lot of muons, is it worth it in terms of signal to background trade off? TO DISCUSS
  
  double mu_from_Z_eta_Cut = 2.5;
  
  double mu_from_upsi_eta_Cut = 2.5;
 ...
 // double mu_from_upsi_RAPIDITY_Cut = 2.4; // keep commented in for the moment so things compile
  ...
  double pT_lep1_Cut = 20;
  
  double pT_lep2_Cut = 10;
  
  double pT_lep3_Cut = 5;
  
  double pT_lep4_Cut = 5;
 ...
 // double mu_mu_from_Z_Prob_Cut = 0.05; 
  ...
  TFile *ntuple = new TFile("ntuple_skimmed_inputFileIs_Run2018_Total_4Jan2022_big4MuMass_allMuonsTight_TrigMatchingReqRemoved_lepSepImposed_IPSigCutImposed_secondQuadArrangementAdded_thirdQuadArrangementAdded.root", "RECREATE");
  TTree *aux;
  ...
  aux->Branch("upsi_type", &upsi_type); //useful only in the doMCTruthMatching scenario
  
  aux->Branch("big4MuMass", &big4MuMass); //useful only when running to look at Z->4 mu 
...
///////////////////////////
  ...
  for(int iEntry=0; iEntry<entries; iEntry++) {
    ...
    std::vector<double> temp_big4MuMass;
...
//       }
       ...
       h_cutflow_allQuadCuts->Fill(5);
       
       if (fabs(TREE->lepton1_impactParameterSignificance->at(i)) > mu_from_Z_3DIPSig_Cut || fabs(TREE->lepton2_impactParameterSignificance->at(i)) > mu_from_Z_3DIPSig_Cut  ||  fabs(TREE->lepton3_impactParameterSignificance->at(i)) > mu_from_Z_3DIPSig_Cut ||  fabs(TREE->lepton4_impactParameterSignificance->at(i)) > mu_from_Z_3DIPSig_Cut){
         continue;
       } 
       
       h_cutflow_allQuadCuts->Fill(6);
       //////////////////////////////////////////////
     ...
     //Note to self: see here: http://www.hep.shef.ac.uk/edaw/PHY206/Site/2012_course_files/phy206rlec7.pdf, equation 6, for rapidity definition (on page 5)
      ...
      //If we remove the ambiguous quad pairing cut, these categories are not mutually exclusive. However, as Greg astutely pointed out, only a max of two of the three arrangements will ever work for a given quad (due to there being only two ways to pair up the pluses and minuses to get net 0)
...
//         std::cout << (lepton1 + lepton2).M()<< std::endl; 
         ...
         //Since Z_mass_low, Z_mass_high = upsi_mass_low_phase1, upsi_mass_high_phase1, this if statement is identical to the if statement above. For the moment, make sure we don't waste time running over the identical second if statements by manually setting them to false when they appear below. This comment applies to all the second 
         ...
         //testing
        // Z_first_upsi_phase1_second_pair_12_34_56 = false;
...
//            }
           ...
           if (TREE->lepton3_isTightMuon->at(i) + TREE->lepton4_isTightMuon->at(i) !=2){
               std::cout << "FAILED mu from upsi must be tight cut" << std::endl;
               FailureCount += 1; 
               continue; 
           }
...
//            }
         ...
         //  upsi_mass = (lepton3 + lepton4).M();
          ...
          //   std::cout << "KANGAROO" << std::endl; 
           //Z and upsi masses
              ...
              temp_big4MuMass.push_back((lepton1 + lepton2 + lepton3 + lepton4).M());
 ...
 //          std::cout << "temp_Z_phi.at(0): " << temp_Z_phi.at(0) << std::endl;
      ...
      //      
         ...
         upsi_phase1_first_Z_second_pair_12_34_56 = false; // manually ensure we don't run over this, which would just be running over the same events and applying the same cuts but in a different order (assuming we went through and implemented the cuts they way they were done in the Z->4 mu paper)
...
//             }
            ...
            if (TREE->lepton1_isTightMuon->at(i) + TREE->lepton2_isTightMuon->at(i) !=2){
               std::cout << "FAILED mu from upsi must be tight cut" << std::endl;
               continue; 
           }
...
//             }
      ...
      if (TREE->pair_13_24_56->at(i) == 1){
         ...
         if (Z_first_upsi_phase1_second_pair_13_24_56){
            ...
            if (lepton1.Pt() < lead_mu_from_Z_pT_Cut  || lepton3.Pt() < pT_lep3_Cut){
                std::cout << "FAILED Z mu Pt Cuts" << std::endl;
                continue;
             }    
...
//            }
            ...
            //start upsi cuts 
            if (lepton2.Pt() < pT_lep2_Cut || lepton4.Pt() < pT_lep4_Cut){
              std::cout << "FAILED upsi mu pT cuts!" << std::endl; 
              continue;  
            }
            ...
            if (TREE->lepton2_isTightMuon->at(i) + TREE->lepton4_isTightMuon->at(i) !=2){
                std::cout << "FAILED mu from upsi must be tight cut!" << std::endl;
                continue; 
...
//            }
         ...
         upsi_phase1_first_Z_second_pair_13_24_56 = false; //manually set this to false to avoid running over the same events with the same cuts twice
...
//             }
           ...
           //Upsi cuts
            ...
            if (TREE->lepton1_isTightMuon->at(i) + TREE->lepton3_isTightMuon->at(i) != 2){
               std::cout << "FAILED mu from upsi must be tight cut" << std::endl;
               continue; 
            }
      ...
      if (TREE->pair_14_23_56->at(i) == 1){
       //  std::cout << "TREE->pair_14_23_56->at(i) == 1" << std::endl; 
         ...
         if (Z_first_upsi_phase1_second_pair_14_23_56){
            ...
            if (lepton1.Pt() < lead_mu_from_Z_pT_Cut  || lepton4.Pt() < pT_lep4_Cut){
                std::cout << "FAILED Z mu Pt Cuts" << std::endl;
                continue;
             }  
...
//            }
            ...
            //start upsi cuts 
            if (lepton2.Pt() < pT_lep2_Cut || lepton3.Pt() < pT_lep3_Cut){
               std::cout << "FAILED upsi mu pT cuts" <<std::endl;
               continue;
            }
            ...
            if (TREE->lepton2_isTightMuon->at(i) + TREE->lepton3_isTightMuon->at(i) != 2){
               std::cout << "FAILED mu from upsi must be tight cut!" << std::endl;
               continue; 
            }
...
//                
         ...
         upsi_phase1_first_Z_second_pair_14_23_56 = false; //manually set this to false to avoid running over the same events with the same cuts twice and double counting
...
//             } 
             ...
             if (TREE->lepton1_isTightMuon->at(i) + TREE->lepton4_isTightMuon->at(i) != 2){
                std::cout << "FAILED mu from upsi must be tight cut!" << std::endl; 
                continue ; 
             }
...
//      continue;
  ...
  //the final survivor, one per event at most 
   if (temp_Z_mass.size() == 1  && temp_upsi_mass.size() == 1 && temp_big4MuMass.size() == 1){
     fillCount += 1; 
     std::cout << "DEBUG 2" << std::endl; 
     big4MuMass = temp_big4MuMass.at(0);
