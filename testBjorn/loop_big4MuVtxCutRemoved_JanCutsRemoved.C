...
void run(string file){//, string file2){
  ...
  TH1F *h_dimu_dR = new TH1F ("h_dimu_dR", "h_dimu_dR", 100, 0, 10); h_dimu_dR->SetXTitle("deltaR between dimu vtx vectors");
  h_dimu_dR->Sumw2();
  
  TH1F *h_dimu_dZ = new TH1F ("h_dimu_dZ", "h_dimu_dZ", 100, 0, 10); h_dimu_dZ->SetXTitle("dZ between dimu vtx vectors");
  h_dimu_dZ->Sumw2();
  
  TH2F *h_dimu_dR_vs_dz = new TH2F("h_dimu_dR_vs_dz", "h_dimu_dR_vs_dz", 100, 0, 10, 100, 0, 10); h_dimu_dR_vs_dz->SetXTitle("deltaR between dimu vtx vectors");
  h_dimu_dR_vs_dz->SetYTitle("dZ between dimu vtx vectors");
  h_dimu_dR_vs_dz->Sumw2();
...
//  std::cout << "myAdd(3,5) "<< myAdd(3,5) << std::endl;
 ...
 // double mu_mu_from_Z_Prob_Cut = 0.05; 
  ...
  TFile *ntuple = new TFile("wildCard_ntuple_skimmed_big4muVtxCutRemoved_JanCutsRemoved_27July2022_inputFileIs_12July2022_Run2018_Total_noRecoToTrigMuMatching.root", "RECREATE");
  TTree *aux;
...
/////////////////////////////////////////////////////////
 ...
 // h_big4MuVtxProb_before_big4MuVtx_Prob_Cut->SaveAs("h_big4MuVtxProb_before_big4MuVtx_Prob_Cut.pdf");
  ...
  TCanvas *c_dimu_dR = new TCanvas ("c_dimu_dR", "c_dimu_dR");
  c_dimu_dR->cd();
  h_dimu_dR->Draw();
  h_dimu_dR->Write();
  c_dimu_dR->SaveAs("c_dimu_dR.pdf");
  
  TCanvas *c_dimu_dZ = new TCanvas("c_dimu_dZ", "c_dimu_dZ");
  c_dimu_dZ->cd();
  h_dimu_dZ->Draw();
  h_dimu_dZ->Write();
  c_dimu_dZ->SaveAs("c_dimu_dZ.pdf");
  
  TCanvas *c_dimu_dR_vs_dz = new TCanvas("c_dimu_dR_vs_dz", "c_dimu_dR_vs_dz");
  c_dimu_dR_vs_dz->cd();
  h_dimu_dR_vs_dz->Draw();
  h_dimu_dR_vs_dz->Write();
  c_dimu_dR_vs_dz->SaveAs("c_dimu_dR_vs_dz.pdf");
